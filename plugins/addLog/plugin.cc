...
EventProcessEnum OnGroupMessage(GroupMessageData data)
{
    ...
    ftime(&t);
    // 判断消息内容
    if (RecieveMessage(data.ThisQQ, t.time * 1000 + t.millitm, data.MessageRandom, data.MessageReq, data.MessageGroupQQ, data.SenderQQ, UnescapeUTF8(data.MessageContent).c_str(), ""))
    {
        api->OutputLog("addlog Recieve Block");
        return EventProcessEnum::Block;
    }
        // 未处理过的消息
    api->OutputLog("addlog final Ignore");
    // 已经处理过的消息阻止其他插件继续处理
    return EventProcessEnum::Ignore;
}
